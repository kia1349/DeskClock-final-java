...
import static com.persiancalendar.deskclock.data.Timer.State.EXPIRED;
import static com.persiancalendar.deskclock.data.Timer.State.MISSED;
import static com.persiancalendar.deskclock.data.Timer.State.PAUSED;
import static com.persiancalendar.deskclock.data.Timer.State.RESET;
import static com.persiancalendar.deskclock.data.Timer.State.RUNNING;
...
public final class Timer {
    ...
    public long getRemainingTime() {
        ...
        final long timeSinceStart = Utils.now() - mLastStartTime;
    ...
    Timer start() {
        ...
        return new Timer(mId, RUNNING, mLength, mTotalLength, Utils.now(), Utils.wallClock(), mRemainingTime,
    ...
    Timer expire() {
        ...
        return new Timer(mId, EXPIRED, mLength, 0L, Utils.now(), Utils.wallClock(), remainingTime, mLabel,
                mDeleteAfterUse);
    }
    ...
    Timer miss() {
        ...
        final long remainingTime = Math.min(0L, getRemainingTime());
        return new Timer(mId, MISSED, mLength, 0L, Utils.now(), Utils.wallClock(), remainingTime, mLabel,
                mDeleteAfterUse);
    }
    ...
    Timer updateAfterReboot() {
        ...
        final long timeSinceBoot = Utils.now();
        final long wallClockTime = Utils.wallClock();
